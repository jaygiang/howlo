...
export async function renderBingoCardImage(userId, isBlank = false) {
  try {
    ...
    if (!isBlank) {
      userAccomplishments = await Accomplishment.find({ userId }).exec();
    }
    ...
    for (let i = 0; i < 5; i++) {
      const svgHeader = `
        <svg width="${cellSize}" height="120">
          <rect width="${cellSize}" height="120" fill="none" />
          <text x="${cellSize/2}" y="85" font-family="Arial" font-size="90" font-weight="bold" text-anchor="middle" fill="black">${letters[i]}</text>
        </svg>
      `;
    ...
    for (let row = 0; row < 5; row++) {
      for (let col = 0; col < 5; col++) {
        ...
        const cellColor = isCompleted ? '#D3D3D3' : 'white';
        ...
        if (isCompleted) {
          const checkmarkY = startY + (lines.length * lineHeight) + (bonusText ? 45 : 25);
          checkmarkElement = `<text x="${cellSize/2}" y="${checkmarkY}" 
                                  font-family="Arial" font-size="32" text-anchor="middle" fill="#2e7d32">✓</text>`;
        }
        
        const svgCell = `
          <svg width="${cellSize}" height="${cellSize}">
            <rect width="${cellSize}" height="${cellSize}" rx="15" ry="15" fill="${cellColor}" stroke="${isCompleted ? 'gray' : '#ccc'}" stroke-width="3" />
            ${textElements}
            ${bonusTextElement}
            ${checkmarkElement}
          </svg>
        ...
        const posY = row * (cellSize + padding) + 160; // Offset from header
