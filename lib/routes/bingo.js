...
async function getUsernameFromSlackId(taggedUserText) {
  try {
    ...
    if (
      taggedUserText &&
      taggedUserText.startsWith("<@") &&
      taggedUserText.endsWith(">")
    ) {
      // Extract the user ID
    ...
    console.warn(
      `Error getting username from Slack ID ${taggedUserText}:`,
      error.message,
    );
...
router.get("/card", async (req, res) => {
  ...
  try {
      ...
      userAccomplishments
        .map(async (acc) => {
          try {
            ...
            const challenge = acc.challenge
              ? acc.challenge.toString().trim()
              : "";

            // Get tagged user display name
            const taggedUserDisplay = await getUsernameFromSlackId(
              acc.taggedUser || "",
            );
    ...
    // Add debug logging
    console.log(
      `Processed ${processedAccomplishments.length} accomplishments out of ${userAccomplishments.length} total`,
    );
    ...
    console.log(
      `User has completed ${completedChallenges.size} challenges (including FREE)`,
    );
    ...
    for (let row = 0; row < 5; row++) {
      ...
      for (let col = 0; col < 5; col++) {
        try {
          ...
          console.log(
            `Cell [${row},${col}]: "${trimmedChallenge}" - Completed: ${isCompleted}`,
          );
          ...
          if (isCompleted && trimmedChallenge !== "FREE") {
            ...
            if (accomplishment && accomplishment.taggedUser) {
              try {
                taggedUserDisplay = await getUsernameFromSlackId(
                  accomplishment.taggedUser,
                );
              } catch (err) {
                console.warn(
                  `Error getting username for ${accomplishment.taggedUser}:`,
                  err.message,
                );
    ...
    const totalXp = userAccomplishments.reduce(
      (sum, acc) => sum + (acc.xp || 0),
      0,
    );
    ...
    const currentMonthXp = currentMonthAccomplishments.reduce(
      (sum, acc) => sum + (acc.xp || 0),
      0,
    );
    ...
    const isBlackout = userAccomplishments.some(
      (acc) => acc.blackoutBonus === true,
    );
    ...
    let rankDisplay = "";
    try {
      // Query to get all users sorted by XP for the current month
      ...
      // Calculate rank (add 1 because array indices start at 0)
      const userRank = userIndex !== -1 ? userIndex + 1 : null;

      // Format the rank with appropriate suffix
      if (userRank) {
        let rankSuffix;
        if (userRank % 10 === 1 && userRank % 100 !== 11) rankSuffix = "st";
        else if (userRank % 10 === 2 && userRank % 100 !== 12)
          rankSuffix = "nd";
        else if (userRank % 10 === 3 && userRank % 100 !== 13)
          rankSuffix = "rd";
        else rankSuffix = "th";
        ...
        rankDisplay = `${rankEmoji} ${userRank}<sup>${rankSuffix}</sup>`;
      } else {
        rankDisplay = "Not ranked";
      }
    } catch (error) {
    ...
    let html = `<html>
      <head>
        ...
        <style>
          ...
          .rank-card[data-rank="1"] .stat-value {
            color: gold;
            text-shadow: 0 0 2px rgba(0,0,0,0.3);
          }
          .rank-card[data-rank="2"] .stat-value {
            color: silver;
            text-shadow: 0 0 2px rgba(0,0,0,0.3);
          }
          .rank-card[data-rank="3"] .stat-value {
            color: #cd7f32; /* bronze */
            text-shadow: 0 0 2px rgba(0,0,0,0.3);
          }
        </style>
                  ...
                  <div class="td-content">
                    ...
                    ${
                      ...
                      ${
                        ...
                        ${
                          accomplishment.eventLocation
                            ? `<div class="event-location">${accomplishment.eventLocation}</div>`
                            : ""
                        }
                      `
                          : ""
        ...
        <div class="profile-header">
          <div class="profile-avatar">
            <img src="${
              ...
              "https://howlo.vercel.app/images/coyote.png"
            }" alt="${userName}">
          </div>
          <div class="profile-name">
            ...
            <div class="profile-badges">
              ...
              ${
                isBlackout
                  ? '<span class="badge blackout-badge">🌑 DENOUT</span>'
                  : ""
              }
            </div>
          </div>
        ...
        <div class="stats-dashboard">
          <div class="stats-section">
            ...
            <div class="stats-row">
                ...
                <div class="stat-value">${rankDisplay}</div>
          ...
          <div class="recent-activity">
            ...
            <div class="activity-timeline">
              ${(
                await Promise.all(
                  currentMonthAccomplishments.slice(0, 3).map(async (acc) => {
                    const formattedDate = acc.timestamp
                    ...
                    try {
                      ...
                      console.warn(
                        `Error getting username for recent activity:`,
                        error.message,
                      );
                    }
                  ...
                  <div class="activity-item">
                    ...
                    <div class="activity-content">
                      <div class="activity-title">${
                        acc.challenge || "Unknown challenge"
                      }</div>
