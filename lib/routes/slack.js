...
router.post('/commands', async (req, res) => {
  ...
  if (trimmedText.toLowerCase() === 'leaderboard') {
    ...
    const cardUrl = `${process.env.APP_BASE_URL || 'https://your-app.vercel.app'}/bingo/card?token=${token}`;
    try {
      await slackClient.chat.postEphemeral({
        channel: channel_id,
        user: user_id,
    ...
    const token = generateToken(user_id);
    const cardUrl = `${process.env.APP_BASE_URL || 'https://your-app.vercel.app'}/bingo/blank-card?token=${token}`;
    try {
      await slackClient.chat.postEphemeral({
        channel: channel_id,
        user: user_id,
    ...
    try {
      ...
      if (channel_id) {
        ...
        if (hasBingo) {
          ...
          await slackClient.chat.postMessage({
            channel: channel_id,
            text: `🎉 *HOWLO!* 🎉 <@${user_id}> has completed a line! View their card here: ${process.env.APP_BASE_URL || 'https://your-app.vercel.app'}/bingo/card?token=${generateToken(user_id)}`,
          });
        }
      }
