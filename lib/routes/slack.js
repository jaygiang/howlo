...
router.post('/interactions', async (req, res) => {
  ...
  if (payload.type === "view_submission" && payload.view.callback_id === "bingo_accomplishment") {
    ...
    try {
      ...
      // Get the tag input value from users_select
      const tagInput = payload.view.state.values.tag_block.tag_input;
      
      if (tagInput && tagInput.selected_user) {
        // This is a workspace user selected from the dropdown
        taggedUser = `<@${tagInput.selected_user}>`;
      } else {
        return res.json({
          response_action: "errors",
          errors: {
            tag_block: "Please select a user"
          }
        });
      }
