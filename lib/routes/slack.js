  ...
  if (trimmedText.toLowerCase() === 'leaderboard') {
    ...
    try {
      ...
      *🎮 1. How to Play:*
          ...
          • Check your progress \`/howlo progress\`
  ...
  if (payload.type === "view_submission" && payload.view.callback_id === "bingo_accomplishment") {
    ...
    try {
      ...
      if (channel_id) {
        ...
        let accomplishmentText = `Accomplishment recorded for <@${user_id}>: *"${cleanAchievement}"* with *${taggedUser}*`;
        ...
        if (blackoutAchieved) {
          accomplishmentText += "\n🏆 *BLACKOUT BONUS!* +1000 XP for unlocking all achievements!";
        }
        ...
        if (blackoutAchieved) {
          await slackClient.chat.postMessage({
            channel: channel_id,
            text: `👑 *BLACKOUT ACHIEVED!* 👑 <@${user_id}> has completed ALL achievements! *+1000 XP Bonus!* View their card here: ${buildUrl(process.env.APP_BASE_URL, `howlo/card?token=${generateToken(user_id)}`)}`,
          });
        }
