        ...
        blocks: [
          ...
          },
          {
            ...
            element: {
              ...
              placeholder: {
                type: "plain_text",
                text: "Type @username or any name"
              }
            }
          },
          {
...
router.post('/interactions', async (req, res) => {
  ...
  if (payload.type === "view_submission" && payload.view.callback_id === "bingo_accomplishment") {
    ...
    try {
      ...
      // Get the tag input value
      const tagInput = payload.view.state.values.tag_block.tag_input.value;
      
      // Process tag input - support for both Slack mentions and custom names
      if (tagInput && tagInput.trim() !== '') {
        // If it's a Slack user ID format (from auto-completion)
        if (tagInput.match(/^U[A-Z0-9]+$/)) {
          taggedUser = `<@${tagInput}>`;
        }
        // If it's already a Slack mention format
        else if (tagInput.match(/^<@[A-Z0-9]+>$/)) {
          taggedUser = tagInput;
        }
        // If it starts with @ (user typed @username)
        else if (tagInput.startsWith('@')) {
          // Try to find if this is a valid Slack username
          try {
            // This approach allows for @username format but won't work perfectly
            // since we can't easily convert @username to user ID without user directory access
            taggedUser = tagInput;
          } catch (err) {
            // If we can't find the user, just use what they typed
            taggedUser = tagInput;
          }
        }
        // Any other text entry
        else {
          // Add @ prefix to indicate it's a person
          taggedUser = '@' + tagInput;
        }
      } else {
        return res.json({
