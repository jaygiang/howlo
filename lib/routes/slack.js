...
router.post('/commands', async (req, res) => {
  ...
  if (trimmedText.toLowerCase() === 'leaderboard') {
    ...
    try {
      // Use the same helpMarkdown defined in the help command section
      await slackClient.chat.postEphemeral({
        channel: channel_id,
        user: user_id,
        text: helpMarkdown,
    ...
    try {
      ...
      *🎮 1. What is HOWLO BINGO?*
          ...
          • HOWLO BINGO is a game where you connect with others in meaningful ways
    ...
    try {
      ...
      const userAvatar = userInfo.user.profile.image_32;
  ...
  if (payload.type === "block_actions") {
    ...
    if (action.action_id === "view_profile") {
      try {
        ...
        const userInfo = await slackClient.users.info({ user: user_id });
        const userName = userInfo.user.real_name || userInfo.user.name || `<@${user_id}>`;
        const userAvatar = userInfo?.profile?.image_32 || "https://howlo.vercel.app/images/coyote.png";
    ...
    if (action.action_id === "copy_invite_link") {
      try {
        ...
        // Show a modal with the invite link that the user can copy
        // Note: Using shorter title (under 25 chars) to comply with Slack's requirements
        await slackClient.views.open({
          trigger_id: payload.trigger_id,
          view: {
            type: "modal",
            callback_id: "invite_link_modal",
            title: {
              type: "plain_text",
              text: "Workspace Invite Link", // Shortened title
              emoji: true
            },
            blocks: [
              {
                ...
                "text": {
                  "type": "mrkdwn",
                  "text": "Share this link to invite someone to join:"
                }
              },
              {
                "type": "section",
              ...
              },
              {
                ...
                "elements": [
                  {
                    "type": "mrkdwn",
                    "text": "Copy the link above and share it with the person you tagged."
                  }
                ]
              }
            ],
        ...
        console.error('Error displaying invite link:', error);
        return res.status(200).send();
      }
    }
  ...
  if (payload.type === "view_submission" && payload.view.callback_id === "bingo_accomplishment") {
    ...
    try {
      ...
      if (channel_id) {
        ...
        // Add a small delay before posting the public message 
        // This gives time for the user to process their ephemeral message first
        setTimeout(async () => {
          // Post public announcement message with a slight format change
          try {
            // Notice the slight change in emoji and formatting to differentiate messages
